...
```
Если нужно чтобы у каждой секции было своё оформление добавить доп. класс к div с классом ``main__wrapper`` например ``main__wrapper section-one`` ``main__wrapper section-two`` и.т.д и для каждого этого класса писать свои стили. Данный шаблон не обязателен к использованию также как и названия классов. Анимацию использованую в этой вёрстке можно увидить в файле style.css находящийся в папке css.
